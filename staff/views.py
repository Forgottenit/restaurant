...
def all_reservations(request):
    ...
    while True:
        last_day_of_month = (first_day_current_month +
                             timedelta(days=31)).replace(day=1
                                                         ) - timedelta(days=1)
        reservations = Reservation.objects.filter(
            date__gte=first_day_current_month, date__lte=last_day_of_month
        ).order_by("date", "time")
        ...
        if first_day_current_month > today + timedelta(
            days=365
        ):  # Display reservations for the next 12 months
            break
